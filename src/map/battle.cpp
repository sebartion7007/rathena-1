...
#else
		case NJ_ISSEN:
			wd->damage = 40 * sstatus->str + sstatus->hp * 20 * skill_lv / 100;
			wd->damage2 = 0;
			break;
		case LK_SPIRALPIERCE:
		case ML_SPIRALPIERCE:
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case TK_DOWNKICK:
		case TK_STORMKICK:
			skillratio += 20 + 60 * skill_lv + sstatus->str * 3;
			break;
		case TK_TURNKICK:
		case TK_COUNTER:
			skillratio += -20 + 80 * skill_lv + + sstatus->str * 3;
			break;
		case TK_JUMPKICK:
	...
	switch (skill_id) {
		...
		case TK_COUNTER:
			if(sd && sd->weapontype1 == W_FIST && sd->weapontype2 == W_FIST)
				ATK_ADD(wd.damage, wd.damage2, 35 * pc_checkskill(sd, TK_RUN));
			break;
		case LG_SHIELDPRESS:
			if (sd) {
				int damagevalue = 0;
...
void battle_adjust_conf()
{
	...
	battle_config.max_uplock_aspd = 2000 - battle_config.max_uplock_aspd * 10;
